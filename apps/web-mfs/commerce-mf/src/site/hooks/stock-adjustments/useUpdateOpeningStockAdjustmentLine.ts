...
import { type UpdateStockAdjustmentLinePayload, updateOpeningLine } from '@/site/services/stock-adjustments.service';
import {
...
export function useUpdateOpeningStockAdjustmentLine(
  adjustmentId: string,
  lineId: string,
  ...
  return useMutation<SuccessResponse, AxiosError, UpdateStockAdjustmentLinePayload>({
    ...options,
    mutationFn: (data) => updateOpeningLine(adjustmentId, lineId, data),
    onSuccess: (...args) => {
