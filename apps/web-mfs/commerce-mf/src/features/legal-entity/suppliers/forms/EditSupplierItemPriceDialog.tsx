...
import { useUpdateSupplierItemPrice } from '@/hooks/legal-entity/suppliers';
import {
...
interface EditSupplierItemPriceDialogProps {
  supplierId: string;
  itemId: string;
  currencyCode?: string;
...
export const EditSupplierItemPriceDialog: React.FC<EditSupplierItemPriceDialogProps> = ({
  supplierId,
  itemId,
  currencyCode,
  price,
  ...
  const updateMutation = useUpdateSupplierItemPrice(supplierId, itemId, { onSuccess });
  ...
  return (
    ...
    >
      <FormSection title="Price" contentClassName="block">
        <div className="flex flex-col gap-4">
          <CurrencyField name="unitPrice" label="Unit Price" currencyCode={currencyCode} />
          <DatePicker name="validTo" label="Valid To (optional)" minDate={new Date(price.validFrom)} />
        </div>
      </FormSection>
