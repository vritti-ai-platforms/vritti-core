...
const routes: RouteObject[] = [
  ...
  { path: ':slug/:tab?', element: gated(SITE_OFFERINGS.view, <OfferingDetailPage />) },
  // Three segments, so it never collides with the two-segment tab route above
  { path: ':slug/variants/generate', element: gated(SITE_OFFERINGS.variants.add, <GenerateVariantsPage />) },
  // Ranks below the static `generate` above — React Router prefers a literal segment over a param
  {
    ...
    element: gated(SITE_OFFERINGS.variants.view, <VariantDetailPage />, <VariantDetailSkeleton />),
  },
];
