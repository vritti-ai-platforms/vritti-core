...
import { useAddCompanyIdentifier } from '@/hooks/organization/companies';
import {
  type AddIdentifierFormData,
  addIdentifierSchema,
  COMPANY_IDENTIFIER_TYPE_OPTIONS,
...
export const AddIdentifierDialog: React.FC<AddIdentifierDialogProps> = ({ partyId, onSuccess, onCancel }) => {
  ...
  const createMutation = useAddCompanyIdentifier(partyId, { onSuccess });

  return (
    <Form form={form} mutation={createMutation} resetOnSuccess onCancel={onCancel}>
      <div className="flex flex-col gap-6">
        <FormSection title="Identity document">
          <Select name="idType" label="Type" placeholder="Select type" options={COMPANY_IDENTIFIER_TYPE_OPTIONS} />
          <TextField name="idValue" label="Number" placeholder="e.g. U74999KA2020PTC000000" />
        </FormSection>
