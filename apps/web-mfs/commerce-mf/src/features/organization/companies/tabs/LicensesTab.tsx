...
export const LicensesTab: React.FC<LicensesTabProps> = ({ companyId }) => {
  const queryClient = useQueryClient();
  const confirm = useConfirm();
  const addDialog = useDialog();
  const { data: response, isLoading } = useCompanyLicensesTable(companyId);
  const deleteMutation = useDeleteCompanyLicense(companyId);
            ...
            actions={[
              {
                ...
                icon: Pencil,
                label: 'Edit',
                permission: ORG_COMPANIES.licenses.edit,
                dialog: {
                  title: 'Edit License',
              ...
              },
              {
                ...
                label: 'Delete',
                variant: 'destructive',
                permission: ORG_COMPANIES.licenses.delete,
  ...
  const { table } = useDataTable({
    ...
    slug: `commerce-org-company-${companyId}-licenses`,
    label: 'license',
    enableRowSelection: false,
    enableSorting: true,
    onStatePush: () => queryClient.invalidateQueries({ queryKey: COMPANY_LICENSES_TABLE_KEY(companyId) }),
  });
  ...
  return (
    <>
      <DataTable
        ...
        toolbarActions={{
          actions: (
            <Button
              size="sm"
              permission={ORG_COMPANIES.licenses.add}
        ...
        emptyStateConfig={{
          icon: ScrollText,
          title: 'No licenses',
          description: 'Record regulatory licenses like drug, excise, or FSSAI for this company.',
          action: (
            <Button
              permission={ORG_COMPANIES.licenses.add}
              startAdornment={<Plus className="size-4" />}
      ...
      <Dialog
        ...
        icon={ScrollText}
        title="Add License"
        description="Record a regulatory license for this company."
        content={(close) => <AddLicenseDialog companyId={companyId} onSuccess={close} onCancel={close} />}
      />
    </>
  );
};
