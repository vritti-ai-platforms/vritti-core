...
export const AddressesTab: React.FC<AddressesTabProps> = ({ partyId }) => {
  ...
  const { data: response, isLoading } = usePersonAddresses(partyId);
  const deleteMutation = useRemovePersonAddress(partyId);
            ...
            actions={[
              {
                ...
                icon: Pencil,
                label: 'Edit',
                permission: ORG_PEOPLE.addresses.edit,
                dialog: {
                  title: 'Edit Address',
              ...
              },
              {
                ...
                label: 'Remove',
                variant: 'destructive',
                permission: ORG_PEOPLE.addresses.delete,
  ...
  const { table } = useDataTable({
    ...
    slug: `commerce-org-person-${partyId}-addresses`,
    label: 'address',
    enableRowSelection: false,
    enableSorting: false,
    onStatePush: () => queryClient.invalidateQueries({ queryKey: PERSON_ADDRESSES_TABLE_KEY(partyId) }),
  });
      ...
      <DataTable
        ...
        toolbarActions={{
          actions: (
            <Button
              size="sm"
              permission={ORG_PEOPLE.addresses.add}
        ...
        emptyStateConfig={{
          icon: MapPin,
          title: 'No addresses',
          description: "Record this person's registered, billing, or shipping addresses.",
          action: (
            <Button
              permission={ORG_PEOPLE.addresses.add}
              startAdornment={<Plus className="size-4" />}
