...
export const IdentifiersTab: React.FC<IdentifiersTabProps> = ({ partyId, binding }) => {
  ...
  const columns = useMemo<ColumnDef<PartyIdentifierRow>[]>(
    ...
    [handleDelete, binding.permissions.delete],
  );
  ...
  return (
    <>
      ...
      <Dialog
        ...
        content={(close) => (
          <AddIdentifierDialog partyId={partyId} binding={binding} onSuccess={close} onCancel={close} />
        )}
      />
    </>
  );
