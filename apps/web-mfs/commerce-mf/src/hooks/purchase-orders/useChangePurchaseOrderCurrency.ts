...
export function useChangePurchaseOrderCurrency(
  options?: Omit<
    UseMutationOptions<SuccessResponse, AxiosError, { id: string; currencyCode: string; conversionRate: number }>,
    'mutationFn'
  >,
) {
  const queryClient = useQueryClient();

  return useMutation<SuccessResponse, AxiosError, { id: string; currencyCode: string; conversionRate: number }>({
    ...options,
