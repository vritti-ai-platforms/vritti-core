...
import { getInventoryItemBatch, getInventoryItemBatchAdjustmentsTable, getInventoryItemBatchLedgerTable } from '@/services/inventory-item-batches.service';

export const INVENTORY_ITEM_BATCH_KEY = (batchId: string) =>
  ['commerce', 'inventory-item-batches', batchId] as const;

export const INVENTORY_ITEM_BATCH_LEDGER_KEY = (batchId: string) =>
  ['commerce', 'inventory-item-batches', batchId, 'ledger'] as const;

export const INVENTORY_ITEM_BATCH_ADJUSTMENTS_KEY = (batchId: string) =>
  ['commerce', 'inventory-item-batches', batchId, 'adjustments'] as const;
...
export function useInventoryItemBatchAdjustmentsTable(
  batchId: string | null,
  options?: Omit<UseQueryOptions<StockAdjustmentsTableResponse, AxiosError>, 'queryKey' | 'queryFn' | 'enabled'>,
) {
  return useQuery<StockAdjustmentsTableResponse, AxiosError>({
    queryKey: [...INVENTORY_ITEM_BATCH_ADJUSTMENTS_KEY(batchId ?? '')],
    queryFn: () => getInventoryItemBatchAdjustmentsTable(batchId as string),
    enabled: !!batchId,
    ...options,
  });
}
