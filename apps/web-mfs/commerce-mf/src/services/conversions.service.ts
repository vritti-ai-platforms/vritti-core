...
export function createConversion(data: CreateConversionPayload): Promise<ConversionData> {
  return axios.post<ConversionData>('commerce-api/conversions', data).then((r) => r.data);
}

export function getConversion(id: string): Promise<ConversionDetail> {
  return axios.get<ConversionDetail>(`commerce-api/conversions/${id}`, { showSuccessToast: false }).then((r) => r.data);
}

export function completeConversion({ id, locationId }: { id: string; locationId: string }): Promise<SuccessResponse> {
  return axios.patch<SuccessResponse>(`commerce-api/conversions/${id}/complete`, { locationId }).then((r) => r.data);
}
