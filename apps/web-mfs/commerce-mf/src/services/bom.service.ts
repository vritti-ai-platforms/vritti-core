...
export function getBomTable(): Promise<BomTableResponse> {
  return axios.get<BomTableResponse>('commerce-api/bom/table', { showSuccessToast: false }).then((r) => r.data);
}

// Creates a new BOM with lines
export function createBom(data: CreateBomPayload): Promise<BomDetail> {
  return axios.post<BomDetail>('commerce-api/bom', data).then((r) => r.data);
}

// Fetches BOM detail with lines
export function getBom(id: string): Promise<BomDetail> {
  return axios.get<BomDetail>(`commerce-api/bom/${id}`, { showSuccessToast: false }).then((r) => r.data);
}
...
export function updateBom({ id, data }: { id: string; data: UpdateBomPayload }): Promise<BomDetail> {
  return axios.patch<BomDetail>(`commerce-api/bom/${id}`, data).then((r) => r.data);
}

// Deletes a BOM
export function deleteBom(id: string): Promise<SuccessResponse> {
  return axios.delete<SuccessResponse>(`commerce-api/bom/${id}`).then((r) => r.data);
}
