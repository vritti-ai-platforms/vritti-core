...
-- `commerce` schema. commerce-service shares core-server's database (vritti_local) but owns its own
-- `commerce` schema, so it grants that schema here — the mirror of core-server's core-schema grant.
--
-- Mirrors what runMigrationsAndGrants applies on deploy. Apply it AFTER commerce migrations, as the
-- owner, via `pnpm db:grant`. Guarded + idempotent: a no-op until the role and schema exist.
DO $$
DECLARE
  target_role   text := 'vritti_core_app';
  target_schema text := 'commerce';
BEGIN
