ALTER TABLE "commerce"."offering_dimension_templates" ALTER COLUMN "legal_entity_id" SET DEFAULT cast(nullif(current_setting('app.le_id', true), '') as uuid);--> statement-breakpoint
ALTER TABLE "commerce"."offering_dimension_templates" DROP CONSTRAINT "offering_dimension_templates_owner_chk", ADD CONSTRAINT "offering_dimension_templates_owner_chk" CHECK ("site_id" is null or "legal_entity_id" is not null);--> statement-breakpoint
ALTER POLICY "reach_read" ON "commerce"."offering_dimension_templates" TO public USING (
        ...
        or (cast(nullif(current_setting('app.site_id', true), '') as uuid) is null and cast(nullif(current_setting('app.le_id', true), '') as uuid) is null)
