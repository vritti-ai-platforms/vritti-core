...
@Injectable()
export class InventoryItemLocationsDomainRepository extends PrimaryBaseRepository<typeof inventoryItemLocations> {
  ...
  async findLocationsFeedKeyset(options: { where?: SQL; orderBy: SQL[]; limit: number }): Promise<{
    ...
    return this.findKeyset<InventoryItemLocation & { locationName: string | null; locationPath: string | null }>({
      select: {
        ...
        isPreferred: inventoryItemLocations.isPreferred,
        minLevel: inventoryItemLocations.minLevel,
        maxLevel: inventoryItemLocations.maxLevel,
        safetyStock: inventoryItemLocations.safetyStock,
        binCapacity: inventoryItemLocations.binCapacity,
