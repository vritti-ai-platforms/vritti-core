...
import { GoodsReceiptsPublishService } from '@domain/goods-receipts/services/goods-receipts-publish.service';
...
export class GoodsReceiptsRootController {
  ...
  @MessagePattern({ cmd: 'goodsReceipts.startAllocation' })
  startAllocation(@Payload() data: { id: string }): Promise<GoodsReceiptDto> {
    this.logger.log('goodsReceipts.startAllocation');
    return this.publishService.startAllocation(data.id);
  }
