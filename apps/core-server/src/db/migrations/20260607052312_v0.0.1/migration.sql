...
CREATE TABLE "vritti_core"."business_units" (
	...
	"pick_strategy" "vritti_core"."pick_strategy" DEFAULT 'FEFO'::"vritti_core"."pick_strategy" NOT NULL,
	"metadata" jsonb,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL
);
...
ALTER TABLE "vritti_core"."user_role_assignments" ADD CONSTRAINT "user_role_assignments_granted_by_users_id_fkey" FOREIGN KEY ("granted_by") REFERENCES "vritti_core"."users"("id") ON DELETE SET NULL;--> statement-breakpoint
CREATE POLICY "org_isolation" ON "vritti_core"."business_units" AS PERMISSIVE FOR ALL TO public USING (organization_id = (select nullif(current_setting('app.org_id', true), '')::uuid));