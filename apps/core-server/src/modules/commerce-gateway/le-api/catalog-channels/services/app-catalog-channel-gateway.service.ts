...
const TABLE_SLUG = 'commerce-le-app-channels';

@Injectable()
export class LeAppCatalogChannelGatewayService {
  private readonly logger = new Logger(LeAppCatalogChannelGatewayService.name);
  ...
  async findForTable(userId: string, organizationId: string): Promise<CatalogChannelTableResponseDto> {
    this.logger.log('le.appCatalogChannels.table');
  ...
  async update(channelId: string, dto: UpdateAppChannelDto): Promise<SuccessResponseDto> {
    this.logger.log(`le.appCatalogChannels.update — channelId: ${channelId}`);
    return this.nats.send('commerce', 'le.appCatalogChannels.update', {
      channelId,
      catalogId: dto.catalogId,
    });
  }
  ...
  async remove(channelId: string): Promise<SuccessResponseDto> {
    this.logger.log(`le.appCatalogChannels.delete — channelId: ${channelId}`);
    return this.nats.send('commerce', 'le.appCatalogChannels.delete', { channelId });
  }
  ...
  async findItemsForTable(userId: string, channelId: string): Promise<ChannelItemTableResponseDto> {
    this.logger.log(`le.appCatalogChannels.items — channelId: ${channelId}`);
  ...
  async setItemVisibility(channelId: string, listingId: string, sellsHere: boolean): Promise<SuccessResponseDto> {
    this.logger.log(`le.appCatalogChannels.setItemVisibility — listingId: ${listingId}`);
    return this.nats.send('commerce', 'le.appCatalogChannels.setItemVisibility', {
      channelId,
      listingId,
      sellsHere,
    });
