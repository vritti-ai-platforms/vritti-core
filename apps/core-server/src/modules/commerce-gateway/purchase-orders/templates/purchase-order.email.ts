    ...
    .map((item, index) => {
      ...
      return `
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #e5e7eb; text-align: right;">${item.uomQty}</td>
...
export function buildPurchaseOrderEmailText(po: PurchaseOrderEmailData, supplier: SupplierEmailData): string {
  ...
  const itemLines = po.items
    .map((item, index) => {
      ...
      return `${index + 1}. ${itemName} | Qty: ${item.uomQty} | Unit: ${formatAmount(item.unitPrice)} | Total: ${formatAmount(item.totalPrice)}`;
    })
    .join('\n');
