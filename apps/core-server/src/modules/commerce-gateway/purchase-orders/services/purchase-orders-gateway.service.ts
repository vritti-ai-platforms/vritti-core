...
import { BadRequestException, DataTableStateService, NatsClientService, type SuccessResponseDto } from '@vritti/api-sdk';
...
export class PurchaseOrdersGatewayService {
  ...
  async downloadPdf(id: string, buId: string): Promise<{ buffer: Buffer; filename: string }> {
    ...
    const buffer = await this.buildPurchaseOrderPdf(
      { poNumber: po.poNumber, status: po.status, orderDate: po.orderDate, expectedDate: po.expectedDate, notes: po.notes, totalAmount: po.totalAmount, items: po.items },
      supplier,
      bu?.name ?? null,
    );
  ...
  private async buildPurchaseOrderPdf(po: PurchaseOrderEmailData, supplier: SupplierEmailData, buName: string | null): Promise<Buffer> {
    // Color constants
    ...
    page.drawText(poLabel, { x: PAGE_WIDTH - MARGIN - poLabelWidth, y: headerY, size: 20, font: boldFont, color: DARK });
    const poNumberText = po.poNumber ?? '-';
    const poNumberWidth = font.widthOfTextAtSize(poNumberText, 11);
    page.drawText(poNumberText, { x: PAGE_WIDTH - MARGIN - poNumberWidth, y: headerY - 26, size: 11, font, color: GRAY });
    ...
    // Supplier box
    page.drawRectangle({ x: MARGIN, y: metaY - boxHeight, width: boxWidth, height: boxHeight, color: LIGHT_GRAY, borderColor: BORDER, borderWidth: 1 });
    ...
    page.drawRectangle({ x: detailsBoxX, y: metaY - boxHeight, width: boxWidth, height: boxHeight, color: LIGHT_GRAY, borderColor: BORDER, borderWidth: 1 });
    ...
    const headers = [
      ...
      { label: 'AMOUNT', x: MARGIN + colNum + colDesc + colQty + colUnitPrice + colAmount - 8, align: 'right' as const },
    ];
    ...
    for (const cx of colXPositions) {
      page.drawLine({ start: { x: cx, y: tableTopY }, end: { x: cx, y: tableTopY - rowHeight }, thickness: 0.5, color: GRAY });
    }
    ...
    for (const [index, item] of po.items.entries()) {
      ...
      page.drawLine({ start: { x: MARGIN, y: rowY }, end: { x: MARGIN + CONTENT_WIDTH, y: rowY }, thickness: 0.5, color: BORDER });
      ...
      const amtX = MARGIN + colNum + colDesc + colQty + colUnitPrice + colAmount - 8 - font.widthOfTextAtSize(amtText, 9);
    ...
    // Table bottom border
    page.drawLine({ start: { x: MARGIN, y: currentY }, end: { x: MARGIN + CONTENT_WIDTH, y: currentY }, thickness: 1, color: BORDER });
    ...
    page.drawText(subtotalValue, { x: totalsX + totalsWidth - 10 - subtotalValueWidth, y: totalsY, size: 9, font, color: BLACK });

    // Thin divider
    totalsY -= 14;
    page.drawLine({ start: { x: totalsX, y: totalsY }, end: { x: totalsX + totalsWidth, y: totalsY }, thickness: 0.5, color: BORDER });
    totalsY -= 4;

    // Total row with dark background
    const totalRowHeight = 22;
    page.drawRectangle({ x: totalsX, y: totalsY - totalRowHeight, width: totalsWidth, height: totalRowHeight, color: DARK });
    const totalLabel = 'TOTAL';
    page.drawText(totalLabel, { x: totalsX + 10, y: totalsY - totalRowHeight + 7, size: 10, font: boldFont, color: WHITE });
    const totalValueText = formatAmount(totalAmount);
    const totalValueWidth = boldFont.widthOfTextAtSize(totalValueText, 10);
    page.drawText(totalValueText, { x: totalsX + totalsWidth - 10 - totalValueWidth, y: totalsY - totalRowHeight + 7, size: 10, font: boldFont, color: WHITE });
    ...
    if (po.notes) {
      ...
      page.drawRectangle({ x: MARGIN, y: contentY - notesBoxHeight, width: CONTENT_WIDTH, height: notesBoxHeight, color: LIGHT_GRAY, borderColor: BORDER, borderWidth: 0.5 });
    ...
    contentY -= 14;
    const termsText = 'All goods must be delivered per the specifications in this purchase order. Payment terms are subject to the agreed supplier contract.';
    ...
    page.drawLine({ start: { x: sig1X, y: sigY }, end: { x: sig1X + sigColWidth, y: sigY }, thickness: 0.5, color: DARK });
    ...
    page.drawLine({ start: { x: sig2X, y: sigY }, end: { x: sig2X + sigColWidth, y: sigY }, thickness: 0.5, color: DARK });
