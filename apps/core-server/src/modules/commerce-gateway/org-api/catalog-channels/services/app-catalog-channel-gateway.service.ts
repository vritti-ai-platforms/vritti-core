...
const TABLE_SLUG = 'commerce-org-app-channels';
...
export class AppCatalogChannelGatewayService {
  ...
  async findForTable(userId: string, organizationId: string): Promise<CatalogChannelTableResponseDto> {
    this.logger.log('org.appCatalogChannels.table');
  ...
  async update(channelId: string, dto: UpdateAppChannelDto): Promise<SuccessResponseDto> {
    this.logger.log(`org.appCatalogChannels.update — channelId: ${channelId}`);
    return this.nats.send('commerce', 'org.appCatalogChannels.update', {
      channelId,
      catalogId: dto.catalogId,
    });
  }
  ...
  async remove(channelId: string): Promise<SuccessResponseDto> {
    this.logger.log(`org.appCatalogChannels.delete — channelId: ${channelId}`);
    return this.nats.send('commerce', 'org.appCatalogChannels.delete', { channelId });
  }
  ...
  async findItemsForTable(userId: string, channelId: string): Promise<ChannelItemTableResponseDto> {
    this.logger.log(`org.appCatalogChannels.items — channelId: ${channelId}`);
  ...
  async setItemVisibility(channelId: string, listingId: string, sellsHere: boolean): Promise<SuccessResponseDto> {
    this.logger.log(`org.appCatalogChannels.setItemVisibility — listingId: ${listingId}`);
    return this.nats.send('commerce', 'org.appCatalogChannels.setItemVisibility', {
      channelId,
      listingId,
      sellsHere,
    });
