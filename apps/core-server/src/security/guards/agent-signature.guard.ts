...
export class AgentSignatureGuard implements CanActivate {
  ...
  constructor(private readonly configService: ConfigService) {
    this.publicKey = this.configService.getOrThrow<string>('AGENT_PUBLIC_KEY');
  }

  // Verifies the request's Ed25519 signature (x-timestamp + x-signature) against the deployment agent public key
