...
export function UomConversionCard({ conversion, primaryUomSymbol, onEdit, onDelete }: UomConversionCardProps) {
  return (
    <Card className="gap-0 overflow-hidden p-0">
      <View className="flex-row items-center justify-between gap-3 px-4 pb-3 pt-4">
        <View className="min-w-0 flex-1 gap-1">
          <Text className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">UOM</Text>
      ...
      <View className="flex-row items-center gap-3 px-4 pb-3 pt-3">
        <Text className="text-xs uppercase tracking-wider text-muted-foreground">Conversion</Text>
        <Text className="flex-1 text-right font-mono text-base text-foreground" numberOfLines={1}>
          1 {conversion.uomSymbol} = {conversion.toPrimaryConversionFactor} {primaryUomSymbol ?? '—'}
        </Text>
      </View>
    </Card>
  );
