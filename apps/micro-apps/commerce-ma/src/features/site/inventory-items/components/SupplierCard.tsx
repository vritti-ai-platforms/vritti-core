...
export function SupplierCard({ supplier }: { supplier: Supplier }) {
  ...
  const unitPrice = supplier.unitPrice ? fmt.currency(supplier.unitPrice).primary : '—';
  const minOrder = supplier.minOrderQuantity != null ? String(supplier.minOrderQuantity) : '—';
  const leadTime = supplier.leadTimeDays != null ? `${supplier.leadTimeDays} d` : '—';
      ...
      <View className="flex-row items-start justify-between gap-3 px-4 pb-3 pt-4">
        ...
        <View className="flex-row items-center gap-2">
          {supplier.isPreferred ? (
            <Badge variant="secondary" style={preferredBg ? { backgroundColor: preferredBg } : undefined}>
              <DynamicIcon icon={STAR_ICON} size={11} className="text-success" style={{ width: 11, height: 11 }} />
              <Text className="text-xs font-semibold text-success">Preferred</Text>
            </Badge>
          ) : null}
