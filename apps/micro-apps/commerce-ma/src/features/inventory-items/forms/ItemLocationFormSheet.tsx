...
export const ItemLocationFormSheet = forwardRef<
  BottomSheetRef,
  ItemLocationFormSheetProps
>(({ inventoryItemId, uomSymbol, editing }, ref) => {
  const isEdit = editing != null;
  const [createLocation, { loading: creating, error: createError, reset: resetCreate }] =
    useCreateItemLocation();
  const [updateLocation, { loading: updating, error: updateError, reset: resetUpdate }] =
    useUpdateItemLocation();
  ...
  const seedForm = useCallback(() => {
    form.reset(
      editing
        ? { locationId: editing.locationId, reorderLevel: String(editing.reorderLevel) }
        : EMPTY,
    );
    resetCreate();
    resetUpdate();
