...
export const UomConversionFormSheet = forwardRef<
  BottomSheetRef,
  UomConversionFormSheetProps
>(({ inventoryItemId, primaryUomSymbol, editing }, ref) => {
  ...
  const [updateConversion, { loading: updating, error: updateError, reset: resetUpdate }] =
    useUpdateUomConversion();
        ...
        <View className="gap-4 px-4 pb-4">
          ...
          {isEdit ? (
            ...
            <UomSelector
              ...
              onOptionSelect={(opt) =>
                setSelectedSymbol((opt?.additionals?.symbol as string | undefined) ?? null)
              }
            />
          )}
          <TextField
            name="uomQty"
            label={`Count of ${altSymbol ?? "alt UOM"}`}
