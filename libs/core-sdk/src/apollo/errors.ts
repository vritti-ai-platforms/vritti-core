...
/**
 * Runs an Apollo operation and rethrows its failures as `CoreError`.
 *
...
function toCoreError(error: unknown): unknown {
  ...
  if (ServerError.is(error) && error.statusCode >= 500) {
    return new CoreError('The store is unavailable right now.', undefined, error.statusCode);
  }
 ...
 * neither an error nor a result — surfaced as a `CoreError` rather than an assertion, so a caller sees
 * the same error shape it handles everywhere else.
 */
export function requireData<T>(data: T | null | undefined): T {
  if (data == null) throw new CoreError('Something went wrong.', undefined, undefined);
  return data;
}
