...
6. Brand display name         page titles, email sender, wordmark default
```

The workspace id comes first because every script in the emitted site runs under
`infisical run --`, so a site without one cannot start. It also makes the three
...
1. **Nothing shipped may import `@/payload-types`.** Those types are generated by
   ...
   wrong. `lib/site.ts` and `lib/media.ts` declare the shapes they touch instead.
