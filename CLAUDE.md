...
`@vritti/quantum-ui` and `@vritti/api-sdk` are consumed as **published npm packages** (pinned in each app's `package.json`); their source lives outside this repo at `../quantum-ui` and `../api-sdk`. `@vritti/quantum-ui-native` is still **linked** locally (`link:../quantum-ui-native` override) for core-app/commerce-ma. For local development against unpublished quantum-ui/api-sdk changes, use `pnpm link` or a temporary `overrides:` block (do NOT commit it — CI resolves from the registry), rebuild the package (`pnpm build`), and reload the consuming dev server — a stale bundle causes runtime `undefined` errors.

### quantum-ui components — import per-subpath, never the barrel
`import { Button } from '@vritti/quantum-ui/Button'` (NOT `from '@vritti/quantum-ui'`). Prefer an existing component over hand-rolled HTML; if one's missing, add it via the quantum-ui-architect. Key surfaces: layout (`PageHeader`, `PageContent`, `Card`, `DetailField`, `Tabs`, `DangerZone`, `Empty`, `Sidebar`), data (`DataTable` + `RowActions` + cell comps, `TreeView`, `Sortable`, charts), forms (`Form`+`FormSection`, `TextField`/`Select`/`Switch`/`CurrencyField`/…), overlays (`Dialog`, `DropdownMenu`, `Tooltip`, `Alert`, `Sonner`), gating (`PermissionGate`, `usePermission`), app-local selectors `@/selectors/<entity>` (each app owns its own under `src/selectors/<entity>/`; quantum-ui keeps only the base `Select` plus the static-data selectors at `@vritti/quantum-ui/selects/{currency,locale,timezone,icon,iso-country}`), and hook/util subpaths (`/hooks`, `/format`, `/money`, `/lodash`, `/slug`, `/icons`). The **`permission` prop is built into** `Button` / `Switch` / `CompactSwitch` / `DataTable` / `RowActions` items / `Tabs` `TabItem` / `DangerZone` (which also has `showWarning`) — pass the code, don't wrap in a manual `usePermission(...).granted &&`. Full catalog + gating rules: `.claude/rules/permission-gating.md` and the `vritti-frontend` agent.
...
- `db-schema.md` — Drizzle schema + RLS policy conventions (workspace-scope helpers, no hardcoded table/column names, migration rules)
