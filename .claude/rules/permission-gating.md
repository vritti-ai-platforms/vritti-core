...
| `permission` prop on `Button` / `Switch` / `CompactSwitch` / `RowActions` / `DataTable` | a single **action**, **toggle** or a **table** | gated in place — hidden when not granted, disabled + lock when locked |
